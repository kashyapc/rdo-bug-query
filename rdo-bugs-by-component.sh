...
usage() {
(
echo "Usage:    `basename $0` RDO component-name

Examples: `basename $0` RDO openstack-nova           #list bugs for openstack-nova
          `basename $0` RDO openstack-cinder         #list bugs for openstack-cinder
          `basename $0` RDO openstack-glance         #list bugs for openstack-glance
          `basename $0` RDO openstack-swift          #list bugs for openstack-swift
          `basename $0` RDO openstack-keystone       #list bugs for openstack-keystone
          `basename $0` RDO openstack-neutron        #list bugs for openstack-neutron"
...
echo "===For component $1===="
echo '

This contains a local list of the bugs that are open against OpenStack
components. Bugs are tracked in the Red Hat Bugzilla database at
https://bugzilla.redhat.com/ and to report a new bug you should follow this
link:
...
while read bugno status summary; do
    ...
    if [ "$bugclass" != "$lastclass" ]; then
        ...
        case "$bugclass" in
            NEW)
                echo 'Bugs in NEW, ASSIGNED or ON_DEV state are open and waiting for someone to fix.' ;;
            NEEDINFO)
